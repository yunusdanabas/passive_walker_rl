...
def main():
    ...
    model_file = MODEL_BC_HIP_KNEE_ALTERNATIVES / f"hip_knee_alternatives_mse_{args.steps}steps.eqx"
    ...
    model, loss_history = train_model(obs, labels, loss_type="mse")

    # Save model
    out_file = RESULTS_BC_HIP_KNEE_ALTERNATIVES / f"hip_knee_mse_controller_{args.steps}steps.eqx"
    save_model(model, out_file)
    print(f"[train] Saved trained controller → {out_file}")

    # Save loss history
    if args.plot:
        loss_file = RESULTS_BC_HIP_KNEE_ALTERNATIVES / f"training_loss_history_mse_{args.steps}steps.pkl"
