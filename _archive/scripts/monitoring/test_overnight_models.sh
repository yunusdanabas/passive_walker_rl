#!/bin/bash
# Overnight Model Testing Script

echo "========================================="
echo "OVERNIGHT MODEL TESTING"
echo "========================================="

# Find all completed models
MODELS=$(find experiments/overnight -name "final_model.pth" 2>/dev/null)

if [ -z "$MODELS" ]; then
    echo "❌ No completed models found"
    echo "Run training first: ./start_overnight_training.sh phase1"
    exit 1
fi
...
for model in $MODELS; do
    ...
    if [ ! -z "$SUCCESS_RATE" ] && [ ! -z "$AVG_LENGTH" ]; then
        ...
        if (( $(echo "$SUCCESS_NUM >= 80" | bc -l) )) && (( $(echo "$LENGTH_NUM >= 2000" | bc -l) )); then
            ...
            echo "$EXPERIMENT_NAME,$COMP_SUCCESS_RATE,$COMP_AVG_LENGTH,$COMP_AVG_RETURN" >> best_models.csv
...
# Show summary
if [ -f "best_models.csv" ]; then
    echo "Best performing models:"
    echo "Experiment,Success Rate,Avg Length,Avg Return"
    cat best_models.csv
    echo ""
    echo "To test the best model with GUI:"
    BEST_MODEL=$(head -1 best_models.csv | cut -d',' -f1)
    echo "python play_ppo_model.py --model experiments/overnight/$BEST_MODEL/final_model.pth --episodes 3"
else
    echo "No models met the 20+ second criteria yet."
    echo "Continue training or try different configurations."
fi
