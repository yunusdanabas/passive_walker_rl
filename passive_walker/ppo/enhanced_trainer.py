        ...
        for step in range(self.config.n_steps):
            ...
            if self.reward_curriculum_manager is not None:
                ...
                if current_mode == "research":
                    # Use research mode rewards (already computed by environment)
                    pass
                else:
                    # Use FSM mode rewards (simplified)
                    reward = self._compute_fsm_reward(obs, action, next_obs, done)
