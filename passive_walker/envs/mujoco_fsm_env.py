...
# Passive Walker Environment without falling check
...
class PassiveWalkerEnv(gym.Env):
    ...
    def __init__(self, xml_path, simend=30.0, use_nn_for_hip=False, use_nn_for_knees=False, use_gui=True):
        ...
        self.model = mujoco.MjModel.from_xml_path(str(xml_path))  # Convert Path to string
    ...
    def reset(self):
        ...
        # apply your initial controller / qpos tweaks
        self.demo_hip = self.init_controller()
        # now propagate qpos→xpos, xquat, etc
        ...
        self.fsm_knee1 = self.FSM_KNEE1_STANCE
        self.fsm_knee2 = self.FSM_KNEE2_STANCE
        # now it's safe to read xpos/xquat in _get_obs()
        return self._get_obs()
    
    def _get_obs(self):
        # 1) world x pos
        x = float(self.data.qpos[0])
        # 2) world z pos of the torso
        torso_id = mujoco.mj_name2id(self.model,
                                     mujoco.mjtObj.mjOBJ_BODY,
                                     "torso")
        z = float(self.data.xpos[torso_id, 2])
        # 3) torso pitch (Euler y)
    ...
    def step(self, external_action):
        """
        Advance the simulation by one time slice.
        If use_nn_for_hip is True, the external_action's first element is used for the hip actuator;
        if use_nn_for_knees is True, the second and third elements are used for the left and right knees, respectively.
        If a joint is not controlled by NN, the FSM controller updates that joint.
        """
        ...
        obs = self._get_obs()
        reward = self.data.qpos[0]  # Example reward: forward progress.
