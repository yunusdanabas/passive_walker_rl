...
"""
Train an MLP that outputs [hip, kneeL, kneeR] using Huber loss behaviour cloning.

Usage:
    python -m passive_walker.bc.hip_knee_alternatives.train_huber [--epochs N] [--batch B]
                                                                 [--hidden-size H] [--lr LR]
                                                                 [--gpu] [--plot]
"""
...
def train_nn_controller(model, optimizer, obs, labels, epochs, batch, plot_loss=False):
    ...
    for ep in range(1, epochs + 1):
        ...
        print(f"[HUBER] epoch {ep:02d}  loss={loss:.4f}")

    if plot_loss:
        plot_loss_curve(loss_hist, save=str(DATA_DIR / 'loss_histories' / 'hip_knee_huber_training_loss.png'))
...
def main():
    ...
    out = DATA_DIR / "hip_knee_huber_controller.pkl"
