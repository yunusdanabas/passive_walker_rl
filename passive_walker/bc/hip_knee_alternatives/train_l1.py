"""
Train an MLP that outputs [hip, kneeL, kneeR] using L1 loss behaviour cloning.

Usage:
    python -m passive_walker.bc.hip_knee_alternatives.train_l1 [--epochs N] [--batch B]
                                                              [--hidden-size H] [--lr LR]
                                                              [--gpu] [--plot]
"""
...
def train_nn_controller(model, optimizer, obs, labels, epochs, batch, plot_loss=False):
    ...
    def loss_fn(m, o, y):
        preds = jax.vmap(m)(o)
        return jnp.mean(jnp.abs(preds - y))  # L1 loss: mean absolute error
    ...
    for ep in range(1, epochs + 1):
        ...
        print(f"[L1] epoch {ep:02d}  loss={loss:.4f}")

    if plot_loss:
        plot_loss_curve(loss_hist, save=str(DATA_DIR / 'loss_histories' / 'hip_knee_l1_training_loss.png'))
...
def main():
    ...
    out = DATA_DIR / "hip_knee_l1_controller.pkl"
